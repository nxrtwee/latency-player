...
public class MediaSessionPlugin extends Plugin {
    ...
    private Bitmap urlToBitmap(String url) throws IOException {
        ...
        final boolean dataUrl = url.startsWith("data:");

        if (blobUrl || dataUrl) {
            final String base64Url = url.substring(url.indexOf(",") + 1);
            byte[] decodedString = Base64.decode(base64Url, Base64.DEFAULT);
    ...
    public void setMetadata(PluginCall call) {
        ...
        try {
            JSArray artworks = call.getArray("artwork", new JSArray());
            if (artworks.length() > 0) {
                final JSONObject currentArtwork = (JSONObject) artworks.get(0);
                if (currentArtwork.has("src")) {
                    final String artworkSrc = currentArtwork.getString("src");
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            try {
                                artwork = urlToBitmap(artworkSrc);
                                if (service != null) {
                                    updateServiceMetadata();
                                }
                            } catch (IOException e) {
            ...
            Log.e(TAG, "Error parsing artwork: " + e.getMessage());
        }
    ...
    public void setLiked(PluginCall call) {
        isLiked = call.getBoolean("liked", false);
        if (service != null) {
            service.setLiked(isLiked);
        }
        call.resolve();
