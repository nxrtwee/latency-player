...
|-----------|--------|------|-----------|
| **Windows** | 1.1.0 | `Latency-1.1.0-x64.exe` (установщик) или `-portable.exe` | запустить установщик |
| **Linux** | 1.1.0 | `Latency-1.1.0-x86_64.AppImage` (универсальный) или `Latency-1.1.0-x86_64.rpm` (Fedora/RHEL) | AppImage — `chmod +x` и запустить; rpm — `sudo dnf install` |
| **Android** | 1.1.0 | `Latency-1.1.0.apk` | включить «установка из неизвестных источников» → открыть APK |
| **iOS** | 1.1.0 | `Latency-1.1.0-unsigned.ipa` | подписать своим Apple ID через [AltStore](https://altstore.io/) / [Sideloadly](https://sideloadly.io/) (бесплатный аккаунт — перевыпуск раз в 7 дней) |

> 🖥️ **Все платформы теперь под единой версией.** Текущая версия — **1.1.0**.
> Мобильные сборки (Android/iOS) подтянуты почти до паритета с десктопом:
> Яндекс Музыка, «Моя волна», поиск по строчке, альбомы и др.
