...
|-----------|--------|------|-----------|
| **Windows** | 0.6.0 | `Latency-0.6.0-x64.exe` (установщик) или `-portable.exe` | запустить установщик |
| **Android** | 0.6.0 | `Latency-0.6.0.apk` | включить «установка из неизвестных источников» → открыть APK |
| **iOS** | 0.6.0 | `Latency-0.6.0-unsigned.ipa` | подписать своим Apple ID через [AltStore](https://altstore.io/) / [Sideloadly](https://sideloadly.io/) (бесплатный аккаунт — перевыпуск раз в 7 дней) |

> 🖥️ **Все платформы теперь под единой версией.** Текущая версия — **0.6.0**.
