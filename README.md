...
|-----------|--------|------|-----------|
| **Windows** | 1.6.0 | `Latency-1.6.0-x64.exe` (установщик) или `-portable.exe` | запустить установщик |
| **Linux** | 1.6.0 | `Latency-1.6.0-x86_64.AppImage` (универсальный) или `Latency-1.6.0-x86_64.rpm` (Fedora/RHEL) | AppImage — `chmod +x` и запустить; rpm — `sudo dnf install` |
| **Android** | 1.6.0 | `Latency-1.6.0.apk` | включить «установка из неизвестных источников» → открыть APK |
| **iOS** | 1.6.0 | `Latency-1.6.0-unsigned.ipa` | подписать своим Apple ID через [AltStore](https://altstore.io/) / [Sideloadly](https://sideloadly.io/) (бесплатный аккаунт — перевыпуск раз в 7 дней) |

> 🖥️ **Все платформы теперь под единой версией.** Текущая версия — **1.6.0**.
> Мобильные сборки (Android/iOS) с 1.6.0 рисуют **сам десктопный интерфейс**
> (скин nextgen, раскладка Universal) в нативной обвязке — нижние табы, сайдбар
> в ящике, капсула плеера над табами.
