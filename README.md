...
|-----------|--------|------|-----------|
| **Windows** | 1.0.0 | `Latency-1.0.0-x64.exe` (установщик) или `-portable.exe` | запустить установщик |
| **Android** | 1.0.0 | `Latency-1.0.0.apk` | включить «установка из неизвестных источников» → открыть APK |
| **iOS** | 1.0.0 | `Latency-1.0.0-unsigned.ipa` | подписать своим Apple ID через [AltStore](https://altstore.io/) / [Sideloadly](https://sideloadly.io/) (бесплатный аккаунт — перевыпуск раз в 7 дней) |

> 🖥️ **Все платформы теперь под единой версией.** Текущая версия — **1.0.0**.
